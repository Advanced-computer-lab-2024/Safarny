...
const updateTransportsByAdvertiserId = async (req, res) => {
  try {
    ...
    if (updatedTransports.modifiedCount === 0) {
      return res
        .status(404)
        .json({ message: "No transports found for this advertiser" });
    }
    ...
    res
      .status(500)
      .json({ message: "Error updating transports by advertiser", error });
  }
};
// Get all transports
...
const getTransportsByAdvertiserId = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching transports by advertiser", error });
  }
};
